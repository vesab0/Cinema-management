...
export default function Table<T extends Record<string, unknown>>({
  ...
  const flashSaved = (rowIndex: number) => {
    setSaved((prev) => ({ ...prev, [rowIndex]: true }));
    setTimeout(() => setSaved((prev) => ({ ...prev, [rowIndex]: false })), 1800);
  };
  ...
  // Dropdowns autosave directly in the table
    ...
    <>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                {columns.map((col) => (
                  <th
                    key={String(col.key)}
                    className="text-left text-xs font-semibold text-gray-400 uppercase tracking-widest px-5 py-3"
                  >
                    {col.label}
                  </th>
                ))}
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={columns.length + 1} className="text-center text-gray-400 text-sm py-12">
                    No rows yet.
                  </td>
                </tr>
              ) : (
                rows.map((row, rowIndex) => (
                  <tr
                    key={rowIndex}
                    className="border-b border-gray-100 last:border-0 even:bg-gray-50 hover:bg-gray-100 transition-colors"
                  >
                    {columns.map((col) => (
                    ...
                    <td className="px-5 py-2.5">
                      <div className="flex justify-end items-center gap-1">
                        ...
                        <button
                          ...
                          title="Edit"
                          className="p-1.5 rounded-md text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors active:scale-95"
                        >
                        ...
                        <button
                          ...
                          title="Delete"
                          className="p-1.5 rounded-md text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors active:scale-95"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {editingRow !== null && (
        <Modal
          ...
          columns={columns.filter((col) => col.type !== "select")}
          row={editingRow}
          onChange={(key, value) =>
      ...
      {creatingRow !== null && (
        <Modal
          ...
          row={creatingRow}
          onChange={(key, value) =>
            setCreatingRow((prev) => prev ? { ...prev, [key]: value } : prev)
          }
    ...
    >
      ...
      >
        ...
        <div className="flex justify-end gap-2 mt-6">
          ...
          <button
            onClick={onConfirm}
            className="text-sm font-medium px-4 py-2 rounded-lg bg-gray-500 text-white hover:bg-gray-600 active:scale-95 transition-all"
          >
            {confirmLabel}
          </button>
        </div>
